"""Unit tests for stereo 3D handler dual-send behavior."""
...
from bot.handlers import (
    _handle_postdownload_stereo_3d,
    handle_audio_3d_selection,
    handle_postdownload_stereo_3d_intensity_callback,
)


def _callback_update(callback_data="audio_3d:medio"):
    update = MagicMock()
...
def mock_context():
    ...
    context.user_data = {
        ...
        "effect_audio_correlation_id": "corr-3d",
        "effect_type": "stereo_3d",
    }
...
class TestHandleAudio3dSelectionDualSend:
    @pytest.mark.asyncio
    async def test_sends_audio_and_document(self, mock_context):
        ...
        with patch("bot.handlers.TempManager") as temp_mgr_cls, patch(
            ...
            effects = MagicMock()
            effects.stereo_3d.return_value = effects
            ...
            await handle_audio_3d_selection(update, mock_context)
...
class TestPostdownloadStereo3dDualSend:
    @pytest.mark.asyncio
    async def test_sends_audio_and_document(self, mock_context):
        update = _callback_update("postdownload:stereo_3d_intensity:corr:medio")
        ...
        with patch("bot.handlers.TempManager") as temp_mgr_cls, patch(
            ...
            await _handle_postdownload_stereo_3d(
                update, mock_context, entry, "corr", "medio", "Medio"
            )
    ...
    @pytest.mark.asyncio
    async def test_postdownload_stereo_3d_surfaces_audio_effects_error(self, mock_context):
        update = _callback_update("postdownload:stereo_3d_intensity:corr:medio")
        entry = SimpleNamespace(file_path="/tmp/source.mp3")
        ...
        with patch("bot.handlers.TempManager") as temp_mgr_cls, patch(
            ...
            loop = MagicMock()
            loop.run_in_executor = AsyncMock(
                side_effect=AudioEffectsError("Error aplicando efecto 3D: ffmpeg failed")
            )
            loop_mock.return_value = loop
        ...
        error_text = update.callback_query.edit_message_text.await_args_list[-1][0][0]
        assert "Error aplicando efecto 3D: ffmpeg failed" in error_text


class TestPostdownloadStereo3dIntensityCallbackValidation:
    @pytest.mark.asyncio
    async def test_malformed_callback_data_shows_invalid_selection_error(self, mock_context):
        ...
        await handle_postdownload_stereo_3d_intensity_callback(update, mock_context)
    ...
    async def test_unknown_intensity_shows_invalid_intensity_error(self, mock_context):
        update = _callback_update("postdownload:stereo_3d_intensity:corr:extreme")
